<template>
  <div class="ai-questions-container" :class="{ 'eye-care': isEyeCareMode }">
    <div id="stars"></div>
    <div id="stars2"></div>
    <div id="stars3"></div>
      ...
      <div class="question-content" v-if="!isComplete && questions.length > 0">
        ...
        <div class="question-body">
          ...
          <div class="scenario-question" v-if="currentQuestion.type === '场景题'">
            ...
            ></textarea>
            <div class="answer-length-hint">
              {{ String(currentQuestion.userAnswer || '').length }}/1000
            </div>
          </div>
      ...
      <div class="completion-page" v-if="isComplete">
        <div class="completion-card">
          ...
          <div class="completion-summary">
            本轮试题结果已经提交。下一步会先回到 AI 对话界面，由 AI 统一给出分数说明、建议和推荐操作。
          </div>

          <div class="completion-actions">
            <button class="complete-button" :disabled="isReturningToAi" @click="goToAudioAssessment">
              返回AIview中...
            </button>
          </div>
        </div>
...
import {
  ...
  getJobInfo,
  getQuestionAnalysisByAi,
  saveAiQuestionHistory,
  scoreScenarioQuestions
} from '@/utils/api/pages/ai-questions';
...
const buildNextComprehensiveStepUrl = () => {
  ...
  query.push('completedType=questions');
  query.push('mode=COMPREHENSIVE');
  query.push(`score=${encodeURIComponent(String(score.value || 0))}`);
  ...
  try {
    ...
    try {
      const historyData = {
        ...
        passScore: getStagePassScore('questions'),
        ...
        console.log('[AI Questions] 准备保存流程归档:', historyData);
      
      const result = await saveAiQuestionHistory(historyData);
        console.log('[AI Questions] 流程归档保存成功:', result);
      ...
      uni.showToast({
        ...
        console.error('[AI Questions] 保存流程归档失败:', historyError);
        // 流程归档失败不影响主流程，只记录错误
      uni.showToast({
          title: '流程归档失败，但不影响评测结果',
        icon: 'none',
        duration: 2000
      });
    }
    ...
    uni.showToast({ title: '试题作答已完成', icon: 'success', duration: 800 });
    try {
      uni.redirectTo({
        url: targetUrl,
        success: () => {},
...
// 处理交卷逻辑
const normalizeQuestionStageScore = (earnedScore = 0, maxScore = 100) => {
  const safeMaxScore = Math.max(1, Number(maxScore) || 100);
  const safeEarnedScore = Math.max(0, Number(earnedScore) || 0);
  return Math.max(0, Math.min(100, Math.round((safeEarnedScore / safeMaxScore) * 100)));
};
...
async function handleExamSubmission() {
  ...
  let localQuestions = questions.value.map(q => {
    totalPossibleScore += Number(q.score || 0);
    if (q.type === '单选题') {
      // 选择题：比较选项文本是否一致
  ...
  score.value = normalizeQuestionStageScore(totalScore, totalPossibleScore);
...
.eye-care .answer-length-hint {
  color: rgba(46, 125, 50, 0.62);
}
