<template>
  <div class="comprehensive-resume-container" :class="{ 'eye-care': isEyeCareMode }">
    <!-- 科幻背景效果 -->
    ...
    <div class="content-card">
      ...
      <div class="analysis-section" v-else-if="resumeFile && (!analysisComplete || isAnalyzing)">
        ...
        <div class="status-display">
          ...
          <div v-else-if="analysisComplete" class="analysis-complete-message">
            <SmartIcon name="fas fa-check-circle success-icon" />
            <p>简历分析已完成！</p>
            </div>
            </div>
      ...
      <!-- 分析结果显示区域 -->
      <div class="results-section" v-else-if="analysisComplete">
            <div class="result-header">
          <h2>评测结果</h2>
          <div class="score-badge" :class="getScoreClass">{{ resumeScore }}分</div>
              </div>
        ...
        <div class="match-breakdown">
          <h3>匹配度详情</h3>
            ...
            <div class="match-bar-container">
              <div class="match-bar-fill" :style="{ width: `${skillMatch}%` }"></div>
                  </div>
            <span class="match-value">{{ skillMatch }}%</span>
                </div>
                <div class="match-item">
            <span class="match-label">经验匹配度:</span>
            <div class="match-bar-container">
              <div class="match-bar-fill" :style="{ width: `${experienceMatch}%` }"></div>
                  </div>
            <span class="match-value">{{ experienceMatch }}%</span>
                </div>
                <div class="match-item">
            <span class="match-label">教育背景匹配度:</span>
            <div class="match-bar-container">
              <div class="match-bar-fill" :style="{ width: `${educationMatch}%` }"></div>
                  </div>
            <span class="match-value">{{ educationMatch }}%</span>
                </div>
          <div class="match-item">
            <span class="match-label">沟通能力:</span>
            <div class="match-bar-container">
              <div class="match-bar-fill" :style="{ width: `${communicationSkill}%` }"></div>
            </div>
            <span class="match-value">{{ communicationSkill }}%</span>
          </div>
          <div class="match-item">
            <span class="match-label">领导力:</span>
            <div class="match-bar-container">
              <div class="match-bar-fill" :style="{ width: `${leadershipSkill}%` }"></div>
            </div>
            <span class="match-value">{{ leadershipSkill }}%</span>
                </div>
              </div>
        ...
        <div class="suggestions-list">
          <h3>AI改进建议</h3>
            ...
            <li v-for="(suggestion, index) in suggestions" :key="index" class="suggestion-item">{{ suggestion }}</li>
                </ul>
        </div>

        <div class="action-buttons-results">
          <button class="view-report-button" @click="viewReport">
            完成简历评测
          </button>
        </div>
      </div>
...
const experienceMatch = ref(0);
const educationMatch = ref(0);
const communicationSkill = ref(0);
const leadershipSkill = ref(0);
...
const startAnalysis = async () => {
  ...
  try {
    ...
    const updateProgress = () => {
      setTimeout(() => {
        if (progress.value < 30) {
          ...
          progress.value = 99; // 保持在99%直到完成
          currentStep.value = 4;
          currentStatus.value = analysisTips[3];
        }
    ...
    // 使用讯飞返回的分析结果
    resumeScore.value = analysisResult.score || 0;
    skillMatch.value = analysisResult.skillMatch || 0;
    experienceMatch.value = analysisResult.experienceMatch || 0;
    educationMatch.value = analysisResult.educationMatch || 0;
    communicationSkill.value = analysisResult.communicationSkill || 0;
    leadershipSkill.value = analysisResult.leadershipSkill || 0;
    suggestions.value = analysisResult.suggestions || analysisResult.improvementSuggestions || analysisResult.aiSuggestions || analysisResult.advice || [];
    
    // 检查分析结果是否有效
    if (resumeScore.value === 0 && skillMatch.value === 0 && experienceMatch.value === 0) {
  ...
  try {
    ...
    try {
      const historyData = {
        ...
        skillMatch: skillMatch.value,
        experienceMatch: experienceMatch.value,
        educationMatch: educationMatch.value,
        communicationSkill: communicationSkill.value,
        leadershipSkill: leadershipSkill.value,
        aiAnalysis: `简历总体评分：${resumeScore.value}分。技能匹配度：${skillMatch.value}%，经验匹配度：${experienceMatch.value}%，教育背景匹配度：${educationMatch.value}%，沟通能力：${communicationSkill.value}%，领导力：${leadershipSkill.value}%。`,
      ...
      console.log('[Comprehensive Resume] 准备保存历史记录:', historyData);
      
      const result = await saveComprehensiveResumeHistory(historyData);
      console.log('[Comprehensive Resume] 历史记录保存成功:', result);
      ...
      console.error('[Comprehensive Resume] 保存历史记录失败:', historyError);
...
.comprehensive-resume-container {
  display: flex;
