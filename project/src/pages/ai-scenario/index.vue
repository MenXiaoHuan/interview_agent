    ...
    <div class="content-wrapper">
      ...
      <div class="scene-assessment-card" v-if="!isComplete && currentQuestion">
        <div class="card-header">
          <h2 class="card-title">{{ currentRoundMeta.title }}</h2>
          <p class="card-subtitle">{{ currentRoundMeta.subtitle }}</p>
        </div>
        ...
        <div class="recorder-section">
          ...
          <div class="recording-status-wrapper">
            ...
            <div class="recording-status" v-if="!isRecording && audioPath">
              <span aria-hidden="true">✓</span> 已录制 {{ formatTime(audioDuration) }}
            </div>
          </div>
      ...
      <div class="completion-page" v-if="isComplete">
        <div class="completion-card">
              ...
              <div class="question-card" v-for="(q, qIndex) in aiAssessment.questions" :key="qIndex">
                <div class="question-header" @click="toggleQuestion(qIndex)">
                  ...
                  <div class="question-toggle">
                    <span aria-hidden="true">{{ expandedQuestionIndex === qIndex ? '▲' : '▼' }}</span>
                  </div>
                </div>
          ...
          <div class="completion-actions">
            <button class="complete-button full-width" :disabled="isReturningToAi" @click="goToMainComprehensivePage">
              <span aria-hidden="true">✓</span> 返回AIview中...
            </button>
          </div>
        </div>
...
import { generateScenarioQuestionByAi, analyzeComprehensiveScenarioAudio, synthesizeSpeech, uploadAudioFile, createTranscriptionTask, queryTranscriptionTask, saveAiInterviewRoundHistory, BASE_URL } from '@/utils/api/pages/ai-scenario';
...
const roundType = computed(() => normalizeComprehensiveStage(route.query.round || 'round_1') || 'round_1');
const roundMetaMap = {
  round_1: {
    title: '技术一面',
    subtitle: '请根据以下技术场景进行语音回答，系统将评估你的基础能力、项目理解和岗位匹配度'
  },
  round_2: {
    title: '技术二面',
    subtitle: '请结合更深入的技术追问进行回答，系统将评估你的设计取舍、技术深度与压力应对'
  },
  round_3: {
    title: 'HR 三面',
    subtitle: '请根据以下沟通与协作场景进行语音回答，系统将评估你的表达、动机与团队协作能力'
  }
};
const currentRoundMeta = computed(() => roundMetaMap[roundType.value] || roundMetaMap.round_1);
...
const buildRoundCompletionReturnUrl = () => buildInterviewAiReturnUrl({
  completedType: roundType.value,
  mode: 'COMPREHENSIVE',
...
const initializeAssessmentSession = () => {
  ...
  return updateComprehensiveAssessmentState(safeJobId, assessmentId.value, (currentState) => {
    ...
    if (!nextState[roundType.value].completed) {
      nextState[roundType.value].inProgress = true;
      nextState[roundType.value].startTime = nextState[roundType.value].startTime || new Date().toISOString();
    }
    ...
    nextState.overall.currentStage = roundType.value;
    return nextState;
  });
};
...
const handleAssessmentCompletion = async () => {
  ...
  try {
    ...
    const savedSession = updateComprehensiveAssessmentState(safeJobId, assessmentId.value, (currentState) => {
      const nextState = normalizeComprehensiveState(currentState);
      nextState[roundType.value] = {
        ...nextState[roundType.value],
        completed: true,
        inProgress: false,
        score: Math.max(0, Math.min(100, parseInt(overallScore.value, 10) || 0)),
        interviewId: nextState[roundType.value].interviewId || `scene_assessment_${Date.now()}`,
        startTime: nextState[roundType.value].startTime || new Date(Date.now() - totalTime.value * 1000).toISOString(),
        endTime: new Date().toISOString(),
        attempts: Math.max(1, Number(nextState[roundType.value].attempts || 0) + 1)
      };
      nextState.overall = {
        ...nextState.overall,
        startTime: nextState.overall.startTime || new Date(Date.now() - totalTime.value * 1000).toISOString(),
        status: roundType.value === 'round_3' ? 'completed' : 'in_progress',
        currentStage: roundType.value
      };
      return nextState;
    });
...
const goToMainComprehensivePage = async () => {
  ...
  try {
    ...
    const targetUrl = buildRoundCompletionReturnUrl();
    const savedSession = ensureActiveAiAssessmentSession(jobId, {
    ...
    const startTimeISO = savedState[roundType.value]?.startTime || new Date(Date.now() - totalTime.value * 1000).toISOString();
    const params = {
      ...
      passScore: getStagePassScore(roundType.value),
    ...
    try {
      ...
      await saveAiInterviewRoundHistory(params);
    } catch (e) {
    ...
    // 弹窗提示成功
    uni.showToast({
      title: `完成${currentRoundMeta.value.title}成功`,
      icon: 'success',
      duration: 1200
    });
    try {
    ...
    router.replace(buildRoundCompletionReturnUrl());
  }
};
