...
UPDATE job
SET description = '面向推荐、搜索、广告、风控、增长等核心业务场景，负责机器学习模型的建模、训练、评估、上线和持续优化，提升用户体验、业务效率和系统智能化水平。',
    requirements = '岗位职责
...
3. 在顶会、竞赛或开源项目中有高质量成果。'
WHERE id = 1;

UPDATE job
SET description = '负责计算机视觉核心算法研发与落地，覆盖图像理解、目标检测、分割、OCR、多模态理解和视觉生成等方向，支撑业务智能化产品能力建设。',
    requirements = '岗位职责
...
3. 有 CVPR、ICCV、ECCV、NeurIPS 等论文或高影响力开源项目经验。'
WHERE id = 2;

UPDATE job
SET description = '负责自然语言处理和大语言模型相关算法研发，建设文本理解、知识问答、智能对话、信息抽取和内容生成等核心能力。',
    requirements = '岗位职责
...
3. 有 ACL、EMNLP、NAACL、NeurIPS 等论文或高质量开源贡献。'
WHERE id = 3;

UPDATE job
SET description = '负责企业级大数据平台和数据链路建设，支撑离线数仓、实时计算、数据服务、指标体系和数据应用的稳定交付。',
    requirements = '岗位职责
...
3. 有数据资产治理、指标平台或数据服务 API 建设经验。'
WHERE id = 4;

UPDATE job
SET description = '负责业务经营分析、用户增长分析、产品策略分析和数据洞察输出，通过数据驱动业务决策、产品优化和增长策略落地。',
    requirements = '岗位职责
...
3. 有数据产品化、指标平台或自助分析体系建设经验。'
WHERE id = 5;

UPDATE job
SET description = '负责企业级数据架构规划、数据平台技术选型和核心数据模型设计，支撑高并发、大规模、多业务线的数据生产与消费。',
    requirements = '岗位职责
...
3. 有高并发数据服务、实时决策或 AI 数据底座建设经验。'
WHERE id = 6;

UPDATE job
SET description = '负责智能硬件、物联网设备或边缘计算设备的嵌入式软件开发，覆盖驱动、系统、中间件、通信协议和稳定性优化。',
    requirements = '岗位职责
...
3. 有量产项目经验，理解工厂测试、可靠性测试和 OTA 灰度发布。'
WHERE id = 7;

UPDATE job
SET description = '负责物联网平台整体架构设计，打通设备接入、消息通信、规则引擎、设备管理、边缘计算和云端服务，支撑海量设备稳定运行。',
    requirements = '岗位职责
...
3. 有边云协同、数字孪生或设备数据智能分析经验。'
WHERE id = 8;

UPDATE job
SET description = '负责物联网设备、通信链路、云平台和数据安全体系建设，覆盖安全架构、漏洞治理、攻防评估、合规和安全运营。',
    requirements = '岗位职责
...
3. 有 CVE、SRC 高危漏洞、攻防竞赛或安全标准落地经验。'
WHERE id = 9;

UPDATE job
SET description = '负责大型智能系统、业务平台或技术中台的整体架构设计，支撑高并发、高可用、可扩展和可演进的业务系统建设。',
    requirements = '岗位职责
...
3. 有从单体到微服务、从业务系统到平台化的架构演进经验。'
WHERE id = 10;

UPDATE job
SET description = '负责大规模基础设施、业务系统和 AI 平台的智能运维能力建设，提升系统稳定性、交付效率、故障自愈和成本治理水平。',
    requirements = '岗位职责
...
3. 有 AIOps、混沌工程、容量预测或自动化故障恢复项目经验。'
WHERE id = 11;

UPDATE job
SET description = '负责质量保障体系和自动化测试平台建设，覆盖接口、UI、性能、稳定性、CI/CD 和质量数据分析，提升研发交付质量。',
    requirements = '岗位职责
...
3. 有质量平台、精准测试、流量回放、测试数据管理或效能度量建设经验。'
WHERE id = 12;

UPDATE job
SET description = '负责 AI/智能化产品的规划、设计和商业化落地，围绕用户场景、模型能力、数据闭环和产品体验打造可规模化的智能产品。',
    requirements = '岗位职责
...
3. 熟悉 Prompt、RAG、模型评测、AI 安全和人机交互体验设计。'
WHERE id = 13;

UPDATE job
SET name = '传感器开发工程师',
    description = '负责传感器选型、硬件调试、驱动适配、信号处理和可靠性验证，支撑智能硬件、工业设备或物联网终端的感知能力建设。',
    requirements = '岗位职责
...
3. 有量产导入、供应商协同或传感器选型评估经验。'
WHERE id = 14;

UPDATE job
SET description = '负责企业数据治理体系建设，围绕数据标准、数据质量、元数据、主数据、数据安全和资产运营提升数据可信度与使用效率。',
    requirements = '岗位职责
...
3. 熟悉 Atlas、DataHub、Amundsen、DolphinScheduler、DQC 等工具或同类平台。'
WHERE id = 15;

UPDATE job
SET description = '负责智能算法在业务场景中的研究、开发和工程化落地，覆盖优化算法、预测模型、决策智能、推荐排序和智能系统能力建设。',
    requirements = '岗位职责
